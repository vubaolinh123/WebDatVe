        ...
        <div style="margin-bottom : 20px ; border-radius : 20px ; box-shadow:2px 2px 20px #85ff00,2px 2px 20px black"
            ...
            <p>Tình trạng :
                ...
                @elseif ($receipt -> user_view_success == 2)
                    ...
                    <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog"
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <form method="post" action="{{ route('admin.receipt.change_pay') }}">
                                    @csrf
                                    <div class="modal-body">
                                        <input type="hidden" name="id" value="{{ $receipt->id_receipt }}">
                                        <input type="hidden" name="value" value="3">
                                        <textarea name="content_pass" class="form-control textarea" id="textarea_pass"
                                            placeholder="Lý do hoàn tiền" id="" cols="30" rows="10"></textarea>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Thoát</button>
                                        <button type="submit" data-id="{{ $receipt->id_receipt }}" id="res_del"
                                            class="asasas btn btn-primary">Hoàn tiền</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    ...
                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal">Hoàn trả
                        tiền</button>
                ...
                @elseif ($receipt -> user_view_success == 4)
                    <a href="#" class="badge badge-info">Đã hoàn tiền </a>
                    <h2>Lý do :{{ $receipt -> content_pass }}</h2>

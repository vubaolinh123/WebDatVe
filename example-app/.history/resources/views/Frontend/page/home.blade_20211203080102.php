...
<style>
    .edituser{
        ...
        z-index: 10000;
        top:  50% ;left: 50%; background : #fff ; border-radius: 30px;
        padding : 40px;
        display: none;
    }
</style>
    ...
    <div class="edituser row">
        <div class="col-sm-6">
            <form action="">
                <div>
                    <label for="">Tên </label>
                    <input id="input-change-u" type="text" class="form-control" value="{{ auth()->user()->name ?? 'Vui lòng đăng nhập !' }}">
                </div>
                <br>
            ...
            <div class="row">
                <div class="col-sm-3">
                    ...
                    <ul class="nav nav-pills nav-stacked">
                        ...
                        @foreach ($clusterCinema as $cluster)
                        @foreach ($cluster->cinema as $key => $item)
                            <a data-toggle="pill" class="@if ($key + 1 == 1) {{ 'active' }} @endif"  href="#menu{{ $key+1 }}" href="{{ route('web.show.cinema', ['id' => $item->id, 'slug' => \Str::slug($item->name)]) }}"
                                style="padding:20px ; margin : 0 auto" class="row">
                                <div class="col-sm-1">
                                    <img style="width:60px ; height : 60px ; border-radius:50%"
                                        src="{{ URL::to('storage/' . $item->image) }}" alt="">
                ...
                <div class="col-sm-9">
                    <div class="tab-content">
                        @foreach ($clusterCinema as $cluster)
                            @foreach ($cluster->cinema as $key => $item)
                                <div id="menu{{ $key+1 }}" class="tab-pane fade @if ($key + 1 == 1)
                                    {{ "in active" }}
                                @endif">
                                    <h3>Menu {{ $key + 1 }}</h3>

                            ...
                            <tbody>
                                @foreach ($arrRender  as $k =>  $v)
                                    <tr>
                                        ...
                                        <td>{{ $v -> id_showtime  }}</td>
                                        <td>{{ number_format($v -> receipts ->where('user_view_success' , '<' , 4)->sum('total'))}} - {{ count($v -> receipts) }}</td>
                                ...
                                @else
                                    @foreach ($receiptError  as $k =>  $v)
                                        <tr>
                                            ...
                                            <td><a href="{{ route('admin.receipt.show' , ['id' => $v -> id_receipt]) }}"  class="btn btn-primary" >Xem đơn hàng </a></td>

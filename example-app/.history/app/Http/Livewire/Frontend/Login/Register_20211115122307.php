...
class Register extends Component
{
    ...
    public function updatingEmail($email){

        if($this -> email != $email) throw new \ErrorException('Errors');
    }
    ...
    public function ruleCode()
    {
        ...
        if ($check =  TokenGmail::where('id_register', $this->idRes)->first()) {
            if (Hash::check($this->code, $check->activation_code)) {
                ...
                $this -> emit('successGister' , 'Bạn đã đăng kí thành công tài khoản '.$this->email .' và mật khẩu ' . $this->password . ' Hãy ghi nhớ');
                $this-> reset();
            } else {

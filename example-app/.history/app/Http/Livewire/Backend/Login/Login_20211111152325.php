...
class Login extends Component
{
    ...
    public function checkCode(){
        ...
        if( $check =  TokenGmail::where('user_id' , $this -> idTokenLogin)->first()){
            dump($this -> code);
            dump($check->tactivation_code);
            dump(Hash::check($check->tactivation_code,$this -> code));
            dump(Hash::check($this -> code,$check->tactivation_code));
            if( Hash::check($this -> code,$check->tactivation_code)){
                $check -> delete();
                $this -> passNew  = uniqid();

    ...
    public function dashboard()
    {
        ...
        foreach ($receipts as $receipt ){
            ...
            if(!in_array($receipt -> user_id,$arrUser)){
                ...
                $arrSaveUser[$receipt -> user_id] = Receipt::where('user_id',$receipt -> user_id)->count() ;
            }
        }
        ...
        $countFail = Receipt::where('user_view_success',4)->count();
        $countPay = Receipt::where('user_view_success', '<' , 4)->count();
        $countSuccess = Receipt::where('user_view_success',1)->count();

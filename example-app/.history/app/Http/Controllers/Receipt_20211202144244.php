...
class Receipt extends Controller
{
    ...
    public function change_pay(Request $request){
        ...
        if( $request->value == 1){
            ...
            if( $request->value == 0)  $find->update([   'user_view_success' => $request->value]);
            if( $request->value == 3)  $find->update([   'user_view_success' => 3 , 'content_pass' => $request -> content_pass]);
            return 1;
        }
    }
    /**
    ...
    public function render_list(Request $request)
    {
        ...
        if($request->has('showtimes')){
            foreach ($receiptsNew as  $value) {
                // dump( $request->showtimes);
                if(in_array($value->showtime -> id_showtime ,$request->showtimes ) ){
                    array_push($receiptsNew2, $value);
                }
            }
        }else{
            $receiptsNew2 =  $receiptsNew;
        }
        //
        $render = view('Backend.page.receipt.render-list',['receipts' => $receiptsNew2 , 'data' => ( $request -> data ?? 0) ]) -> render();
        return $render;
    }

    ...
    public function search(Request $request)
    {
        ...
        foreach ($films as $film){
            foreach($film -> showtime as $showtime ){
                // if(!in_array($film -> id , $arrId)){
                //     array_push($arrId , $film -> id);
        ...
        foreach($arrSave as $item){
            ?>
                <a href="<?= route('web.detailFim' , ['id_film' => $film -> id_film , 'slug' => Str::slug($item->name)]) ?>" style=" list-style : none "><?= $item -> name ?> <?= $item -> id_film ?></a> <br>
            <?php
        }
    }
}

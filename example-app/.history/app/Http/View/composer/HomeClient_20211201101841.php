...
class HomeClient
{
    ...
    public function compose( View $view)
    {
        ...
        $arrFilm = [];
        $arrCount = [];
        $filmHomeDeleted0s = $this -> film::where('status', 0)->where('deleted', 0)
        ...
        foreach ( $filmHomeDeleted0s  as   $valueByfilmHomeDeleted0s) {
            if(count($valueByfilmHomeDeleted0s-> showtimeNow) > 0 && !in_array($valueByfilmHomeDeleted0s -> id  , $arrCount)){

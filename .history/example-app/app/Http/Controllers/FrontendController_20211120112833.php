...
class FrontendController extends Controller
{
    ...
    public function render_book(Request $request){
       ...
       if($request->session()->has('book1')){
            ...
            dump($dataOld);
            if(array_key_exists($request -> type, $dataOld)){
                if(array_key_exists($request -> id, $dataOld[$request -> type])){
                    dump('No key id');
                    ...
                    dump('No key');
                    $arrCon = [];
                    $arrCon[$request -> id] =  $request -> value  ;
                    $dataOld_new = array_merge($dataOld[$request -> type], $arrCon);
                    $dataOld[$request -> type] =  $dataOld_new;
                    dd($dataOld );
                    $request->session()->put('book1', $dataOld);
                }
            }else{
                dump('No key');
                 $dataOld = array_merge($dataOld, $arr);
                 $request->session()->put('book1', $dataOld);
            }

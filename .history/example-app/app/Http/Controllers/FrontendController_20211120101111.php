...
class FrontendController extends Controller
{
    ...
    public function render_book(Request $request){
        ...
        if(Session::has('book')){
            ...
            $arrOld =  Session::get('book', $arr);
            if(array_key_exists($request-> type, $arrOld)){
                if(array_key_exists($request-> id,$arrOld[$request-> type]) ){
                    $arrOld[$request-> type][$request-> id] = $request-> value;
                }else{
                    $arrD = [];
                    $arrD[$request-> type][$request-> id] = $request-> value;
                    array_merge($arrOld ,$arrD);
                }
            }else{
                $arrD = [];
                $arrD[$request-> type][$request-> id] = $request-> value;
            ...
            Session::put('book', $arrOld);
        }else{
            Session::put('book', $arr);
        }
        dd(Session::get('book', $arr));
    }
}
